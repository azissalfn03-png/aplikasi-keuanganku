...
// ==========================================
// 5. FITUR DETEKTOR INTERNET & PULL-TO-REFRESH
// ==========================================

// --- SENSOR 1: DETEKSI INTERNET MATI / NYALA ---
window.addEventListener('offline', () => {
    // Kalau internet mati, munculkan banner merah jambu
    document.getElementById('offlineBanner').classList.remove('hidden');
});

window.addEventListener('online', () => {
    // Kalau internet nyala lagi, sembunyikan banner dan otomatis ambil data!
    document.getElementById('offlineBanner').classList.add('hidden');
    showToast("Internet kembali terhubung!", "success");
    loadDashboardData(); 
});

// --- SENSOR 2: PULL TO REFRESH (Usap ke bawah) ---
let touchStartY = 0;
let isPulling = false;
const mainElement = document.querySelector('main'); // Target area scroll
const ptrIndicator = document.getElementById('ptrIndicator');
const ptrText = document.getElementById('ptrText');

document.addEventListener('touchstart', (e) => {
    // Hanya aktif kalau posisi layar sedang di paling atas
...
document.addEventListener('touchmove', (e) => {
    ...
    // Kalau ditarik ke bawah lebih dari 60px, munculkan indikator
    if (pullDistance > 60 && mainElement.scrollTop === 0) {
...
document.addEventListener('touchend', (e) => {
    ...
    // Kalau indikatornya lagi muncul, jalankan proses refresh
    if (ptrIndicator.style.top === "20px") {
        ptrText.innerText = "Memperbarui Data...";
        
        // Panggil fungsi utama kamu untuk narik data!
        loadDashboardData().then(() => {
            // Sembunyikan indikator setelah selesai
            setTimeout(() => {
                ptrIndicator.style.top = "-60px"; 
                ptrText.innerText = "Lepas untuk Refresh";
            }, 500);
        });
    }
});
